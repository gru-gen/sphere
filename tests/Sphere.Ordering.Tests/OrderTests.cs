...
public class OrderTests
{
    ...
    private static Order PlaceOne() => Order.Place(
        Guid.NewGuid(),
        [(Guid.NewGuid(), "Aurora Sneaker", Money.Of(89.90m, "EUR"), 2)],
        Clock);
    ...
    public void Place_computes_the_total_and_raises_the_event()
    {
        ...
        var domainEvent = Assert.Single(order.PullDomainEvents());
    ...
    public void Cancel_works_once_and_only_from_placed()
    {
        ...
        Assert.IsType<OrderCancelledDomainEvent>(Assert.Single(order.PullDomainEvents()));
