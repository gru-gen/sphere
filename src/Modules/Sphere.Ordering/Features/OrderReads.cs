...
internal sealed class OrderingReadDb(string connnectionString)
{
    public NpgsqlConnection Open() => new(connnectionString);
}
...
internal static class GetOrder
{
    ...
    internal static async Task<Results<Ok<OrderResponse>, NotFound>> Handle(
        Guid id, OrderingReadDb readDb, CancellationToken cancellationToken)
    {
...
internal static class ListOrders
{
    ...
    internal static async Task<Ok<List<OrderSummaryResponse>>> Handle(
        Guid customerId, OrderingReadDb readDb, CancellationToken cancellationToken)
    {
        await using var connection = readDb.Open();
